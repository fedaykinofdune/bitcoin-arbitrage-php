<?php 
	class KrakenAPI extends API {
		...
		public function __construct() {
			...
			foreach ($data['result']['XXBTXLTC']['asks'] as $bidData) {
				$bidValue = ( 1 / $bidData[0]);
				if($bidValue >= $this->highestBid || false == isset($this->highestBid)) {
			...
			foreach ($data['result']['XXBTXLTC']['bids'] as $askData) {
				$askValue = ( 1 / $askData[0]);
				if($askValue <= $this->lowestAsk || false == isset($this->lowestAsk)) {
					if($askData[1] >= $this->minAcceptableVolume) {
						$this->lowestAsk 		   = $askValue;
		...
		public function getBalance() {
			...
			$version = 0;
			$kraken = new KrakenAPIInterface($this->apikey, $this->apisecret, $url, $version, $sslverify);

			$result = $kraken->QueryPrivate('Balance');

			$this->balanceBTC = $result['result']['XXBT'];
			$this->balanceLTC = $result['result']['XLTC'];
